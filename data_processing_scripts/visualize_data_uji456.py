...
def get_drawing_phase_bounds(df, run_name):
    # Uji 4 tidak memiliki fase menggambar (tidak ada sumbu Z yang turun)
    if "uji4" in run_name.lower():
        return None, None
    ...
    # Bypass jika pergerakan Z sangat kecil (berarti tidak ada hovering -> plunge)
    if z_range < 2.0:
        return None, None
...
def add_shading_and_fix_legend(ax, t_start, t_end):
    """ Helper: Menambah arsiran hijau dan mencegah duplikasi label legend """
...
def plot_twist_control_6dof(df, run_name, out_dir, t_start, t_end):
    ...
    fig.suptitle(f"Analisis Pelacakan Kecepatan 6-DOF (Twist Reference vs Actual) - {run_name}", fontsize=16); fig.tight_layout()
...
def plot_twist_command_saturation_6dof(df, run_name, out_dir, t_start, t_end):
    ...
    fig.suptitle(f"Bukti Saturasi Perangkat Keras (MPC Command vs Actual) - {run_name}", fontsize=16); fig.tight_layout()
...
def plot_estimator_6dof(df, run_name, out_dir, t_start, t_end):
    ...
    fig.suptitle(f"Kinerja Estimator State 6-DOF (Raw Derivatif vs Kalman Filter) - {run_name}", fontsize=16); fig.tight_layout()
...
def plot_overlay_correlation(df, run_name, out_dir, t_start=None, t_end=None):
    ...
    # Tambah Mean Line selama Fase Gambar
    if t_start is not None and t_end is not None:
        mask = (t >= t_start) & (t <= t_end)
        mean_err_pos = df.loc[mask, 'tracking_error_pos_mm'].mean()
        line_mean2 = ax2.plot([t.min(), t.max()], [mean_err_pos, mean_err_pos], color=color2, linestyle='--', alpha=0.5, label=f'Mean (Gambar): {mean_err_pos:.2f} mm')
        line2 += line_mean2
    ...
    if t_start is not None and t_end is not None:
        ...
        line_mean4 = ax4.plot([t.min(), t.max()], [mean_err_ori, mean_err_ori], color=color4, linestyle='--', alpha=0.5, label=f'Mean (Gambar): {mean_err_ori:.2f} deg')
        line4 += line_mean4
...
def plot_joint_velocity_6dof(df, run_name, out_dir, t_start, t_end):
    ...
    axes[0, 0].plot(t, np.degrees(df['js_j1_v']), 'c-', label='Joint 1 Vel'); axes[0, 0].set_ylabel("J1 (deg/s)")
    axes[1, 0].plot(t, np.degrees(df['js_j2_v']), 'c-', label='Joint 2 Vel'); axes[1, 0].set_ylabel("J2 (deg/s)")
    axes[2, 0].plot(t, np.degrees(df['js_j3_v']), 'c-', label='Joint 3 Vel'); axes[2, 0].set_ylabel("J3 (deg/s)"); axes[2, 0].set_xlabel("Waktu (s)")
    axes[0, 1].plot(t, np.degrees(df['js_j4_v']), 'm-', label='Joint 4 Vel'); axes[0, 1].set_ylabel("J4 (deg/s)")
    axes[1, 1].plot(t, np.degrees(df['js_j5_v']), 'm-', label='Joint 5 Vel'); axes[1, 1].set_ylabel("J5 (deg/s)")
    axes[2, 1].plot(t, np.degrees(df['js_j6_v']), 'm-', label='Joint 6 Vel'); axes[2, 1].set_ylabel("J6 (deg/s)"); axes[2, 1].set_xlabel("Waktu (s)")
...
def process_all_dynamic_timeseries():
    ...
    for idx, csv_path in enumerate(csv_files):
        ...
        check_file = out_dir_full / f"{run_name}_4_overlay_correlation.png"
        
        if check_file.exists():
            print(f"[{idx+1}/{total_files}] {run_name} -> SUDAH DIPROSES (Melewati)")
            continue
            
        print(f"[{idx+1}/{total_files}] Memproses: {run_name}")
        
        df = pd.read_csv(csv_path)
        df = calculate_dynamic_local_frame(df)
        
        # Ekstrak rentang waktu Fase Menggambar
        t_start, t_end = get_drawing_phase_bounds(df, run_name)
        
        # Trim data: Potong sisa detik kosong jika t_start ditemukan (Uji 5 & 6)
        if t_start is not None and t_end is not None:
            df, t_start, t_end = trim_timeseries(df, t_start, t_end)
        
        out_dir_full.mkdir(parents=True, exist_ok=True)
        print("  -> Merender Full/Trimmed Timeline...")
        
        if "uji4" not in run_name.lower(): plot_trajectories_2d(df, run_name, out_dir_full)
        ...
        # Zoomed Slices hanya dirender jika panjang rekaman > 30 detik (contoh: Uji 4)
        # Pada Uji 5 & 6, trimming di atas sudah memperpendek durasi, sehingga slicing diabaikan otomatis.
        ...
        if max_time > 30.0:
            ...
            for slice_start in np.arange(0, max_time, window_size):
                ...
                if len(df_slice) > 200:
                    ...
                    if "uji4" not in run_name.lower(): plot_trajectories_2d(df_slice, slice_name, out_dir_zoomed)
